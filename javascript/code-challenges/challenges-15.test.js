...
const screenForNames = (arr) => {
  const regex = /^(Mr|Mrs|Ms|Dr)\.\s\w[\w\s]*$/g;
...
const toTitleCase = (arr) => {
  return arr.map(string => string[0].toUpperCase() + string.substring(1));
};
...
let biggerThanLuke = (arr) => {
  ...
  return arr.reduce((acc, current, i) => {
    if (parseInt(current.mass) > lukeMass) {
      if (!acc) {
        return current.name;
      }
...
const sortBy = (prop, arr) => {
  ...
  } else if (typeof arr[0][prop] === 'number') {
...
const detectTicTacToeWin = (board) => {
  ...
  function helperCheck(one, two, three) {
    const spaces = [one, two, three];
    const checkedMoves = spaces.map(space => {
      return flat[space];
    });
    if (checkedMoves[0] === '') {
      return false;
    }
    return (checkedMoves[0] === checkedMoves[1] && checkedMoves[0] === checkedMoves[2]);
  }

  function checkHorizontal(row) {
    return helperCheck(row, row + 1, row + 2);
  }

  function checkVertical(col) {
    return helperCheck(col, col + 3, col + 6);
  }

  function checkDiags(){
    return helperCheck(0, 4, 8) || helperCheck(2, 4, 6);
  }
