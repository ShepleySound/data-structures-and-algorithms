...
const longestString = (arr) => {
  if (arr.length) {
    return arr.reduce((prev, current, currentIndex) => {
      return arr[prev].length < current.length ? currentIndex : prev;
    }, 0);
  } else return -1;
};
...
const standardizePhoneNumbers = (arr) => {
  return arr.map(string => {
    const first = string.substring(1, 4);
    const second = string.substring(6, 9);
    const third = string.substring(10);
...
const onlyOddChars = (str) => {
  ...
  for (let i in str) {
    if (i % 2 !== 0){
      oddString = oddString.concat(str[i]);
    }
  }
  return oddString;
};
...
const unenrollBrook = (arr) => {
  return arr.map(innerArr => {
    return innerArr.filter(string => {
      return !string.includes('Brook');
    })
  })
};
