...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach(ingredient => {
    const stringArray = [...ingredient];
    const noNumber = stringArray.slice(stringArray.indexOf(' ') + 1);
    const noUnit = noNumber.slice(noNumber.indexOf(' ') + 1);
    const newString = noUnit.join('');
...
const splitFoods = (recipe) => {
  let result = [];
  recipe.ingredients.forEach(ingredient => {
    const wordArray = ingredient.split(' ').slice(2);
