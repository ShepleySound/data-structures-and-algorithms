...
Write a function named transformToList that, given an object, returns an array of the key value pairs as html list items.
...
function transformToLis(obj){
  const listArray = Object.entries(obj).map(([key, val]) => {
...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map(arr => {
    return arr.filter(element => typeof element === 'number' && element % 5 === 0)
      .map(el => Math.pow(2, el))
  });
};
...
let findMaleAndFemale = (data) => {
  let str = '';
  data.filter(element => element.gender === 'male' || element.gender === 'female')
    .forEach((element, i) => {
      if (i) {
        str += ' and ';
      }
...
let findShortest = (data) => {
  const shortestObj = data.reduce((prev, current) => {
